...
describe('RequestHelper.get()', () => {
	it('Should respond with an object', async () => {
		const response = await RequestHelper.get(mockService, 'https://www.test.com');
	...
	it('Should be paginated when links are present', async () => {
		const response = await RequestHelper.get(mockService2, 'https://www.test.com');
	...
	it('Should be paginated but limited by the maxPages option', async () => {
		const response = await RequestHelper.get(mockService2, 'https://www.test.com', { maxPages: 1 });
	...
	it('Should be paginated but limited by the page option', async () => {
		const response = await RequestHelper.get(mockService2, 'https://www.test.com', { page: 2 });
	...
	it('Should show the pagination information when the page option is given', async () => {
		const response = await RequestHelper.get(mockService2, 'https://www.test.com', { page: 2, showPagination: true});
