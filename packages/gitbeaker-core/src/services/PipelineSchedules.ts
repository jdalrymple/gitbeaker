...
export class PipelineSchedules<C extends boolean = false> extends BaseService<C> {
  all(projectId: string | number, options?: PaginatedRequestOptions) {
    ...
    return RequestHelper.get<PipelineScheduleSchema[]>()(this, `projects/${pId}/pipeline_schedules`, options);
  }
  ...
  create(
    ...
    return RequestHelper.post<PipelineScheduleSchema & { variables?: PipelineVariableSchema[] }>()(this, `projects/${pId}/pipeline_schedules`, {
      description,
      ref,
      cron,
      ...options,
  ...
  edit(projectId: string | number, scheduleId: number, options?: BaseRequestOptions) {
    ...
    return RequestHelper.put<PipelineScheduleExtendedSchema>()(this, `projects/${pId}/pipeline_schedules/${sId}`, options);
  }

  remove(projectId: string | number, scheduleId: number, options?: Sudo) {
    const [pId, sId] = [projectId, scheduleId].map(encodeURIComponent);

    return RequestHelper.del<PipelineScheduleExtendedSchema>()(this, `projects/${pId}/pipeline_schedules/${sId}`, options);
  }

  show(projectId: string | number, scheduleId: number, options?: Sudo) {
    const [pId, sId] = [projectId, scheduleId].map(encodeURIComponent);

    return RequestHelper.get<PipelineScheduleExtendedSchema>()(this, `projects/${pId}/pipeline_schedules/${sId}`, options);
  }
